...
The Service API uses [Firebase parameterized configuration](https://firebase.google.com/docs/functions/config-env). Secrets are stored in Google Cloud Secret Manager and bound to the Cloud Function at deploy time. Non-secret configuration uses environment variables.

**Secrets** (set via `firebase functions:secrets:set <NAME>`):
- `ADMIN_PUBLIC_KEY` — RSA public key to validate JWT tokens (multiline PEM — pipe from file: `firebase functions:secrets:set ADMIN_PUBLIC_KEY < key.pem`)
- `AWS_KEY` — AWS IAM user access key
- `AWS_SECRET` — AWS IAM user secret key
- `AWS_ROLE_ARN` — AWS IAM Role ARN with a trust relationship to the IAM user above. This role defines the permissions the token service has in AWS. The token service can only provide credentials to clients that grant permissions which are a subset of this Role's permissions.

**Environment variables** (set in `functions/.env` for local dev, `functions/.env.<projectId>` for deployed environments):
- `AWS_DURATION` — time in seconds that temporary credentials are valid (e.g., `3600`)
...
3. Create `functions/.secret.local` with your development secrets:
   ```
   ADMIN_PUBLIC_KEY=<public key value>
   AWS_KEY=<AWS api key>
   AWS_SECRET=<AWS api secret key>
   AWS_ROLE_ARN=<AWS Role ARN>
   ```
4. Copy the env template (from within `functions/` dir):
   ```
   cp .env.example .env
   ```
5. Set up admin credentials:
  ...
  - Save it as `functions/key.json` — this is gitignored, never commit or share this file
